...
new.cluster.ids <- c("Early makers+ RPC1_d78", "Early makers+ RPC2_d78",
                     "Early makers+ RPC3_d78", "ELAVL2+/4+ RPC2_d78", "ELAVL2+/4+ RPC1_d78",
...
cell_counts <- table(RNA_d78$celltype)
print(cell_counts)
# Early makers+ RPC1_d78      Early makers+ RPC2_d78      Early makers+ RPC3_d78 
# 1248                         680                         650 
# ELAVL2+/4+ RPC2_d78         ELAVL2+/4+ RPC1_d78   ONECUT1+/2+/LHX1+ RPC_d78 
# 472                         416                         360 
# ONECUT1+/2+/PTF1A+ RPC1_d78         ELAVL2+/4+ RPC3_d78    ONECUT1+/MEIS2+ RPC3_d78 
# 328                         323                         303 
# ONECUT1+/2+/PTF1A+ RPC2_d78    ONECUT1+/MEIS2+ RPC1_d78    ONECUT1+/MEIS2+ RPC2_d78 
# 282                         275                         208 
# ONECUT1+/MEIS2+ RPC4_d78          PAX6+/RAX+ RPC_d78             MKI67+ RPC2_d78 
# 183                         173                         133 
# MKI67+ RPC1_d78              VSX2++ RPC_d78             EEF1A1+ RPC_d78 
# 125                         117                          22 
...
celltypes <- levels(RNA_d78)
library(scales)
color_vector <- hue_pal()(length(celltypes))
rna_colors <- setNames(color_vector, celltypes)
show_col(rna_colors)
multiome_d78$celltype <- factor(multiome_d78$predicted.id, levels = names(rna_colors))
...
saveRDS(object = RNA_d78, file = "250829_retina_chromatin_d78_RNAseq_No3.rds")
saveRDS(object = multiome_d78, file = "250829_retina_chromatin_d78_ATACseq_No3.rds")
